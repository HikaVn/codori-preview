...
function createScoreNotation(canvas, options = {}) {
  ...
  function renderFromLayout() {
    ...
    for (let si = 0; si < systems.length; si += 1) {
      ...
      // コード（学習位置・五線の上）。所属（その拍の音符）が分かるよう、コード記号から
      // 所属音符の位置で五線の最上線まで半透明の線で結ぶ。
      if (sys.chords && sys.chords.length) {
        ctx.strokeStyle = "rgba(31,111,79,0.35)"; ctx.lineWidth = 1;
        for (const c of sys.chords) {
          const textX = mapX(c.x);
          const beatX = mapX(c.beatX != null ? c.beatX : c.x);
          ctx.beginPath(); ctx.moveTo(textX, top - 13); ctx.lineTo(beatX, top); ctx.stroke();
        }
