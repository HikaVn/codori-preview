...
m7入門では、正式m7アクション画像ができるまでは、元の白い鳥を使う。
目的は、minorより少しほどける夜の余韻をアプリ上で確認すること。
...
進行練習があるStageは `聞いた / 音あて / 進行` の3つで完了。
進行練習がないStageは `聞いた / 音あて` の2つで完了。
...
2026-05-29時点では、IMAGEGENで白い鳥とワンポイントを一緒に生成した統合版 `assets/app/characters/action-candidate-integrated-v1/` を使う。
