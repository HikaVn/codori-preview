...
音カードの運指図では、基本形に加えて展開形（同じコードの別の押さえ方）を選べる。
展開形は `chord-forms.js` がGCEAチューニングから計算し、運指図SVGを動的に生成する。
基本形はレビュー済みの運指アセットをそのまま使い、展開形を選んだときだけ生成図と実際の押さえ方の音に切り替える。
うた練習のプレイ画面でも、いまのコードの下のボタンで展開形を切り替えられる。
選択はコード名ごとに曲へ保存され、レーンの運指図と自動伴奏の響きにも反映される。
...
通常起動時は、コードカードをすぐには表示しない。
最初にゲームモード選択だけを表示し、`最初から`、`つづける`、`ストーリーリスト`、`コード図鑑へ` のいずれかを選んでからコードを表示する。
モードを選んだ後は、ゲームモード選択画面を消して、選んだモードの本体だけを表示する。
`stage`、`set`、`view`などのURLパラメータがある場合は、確認用の直接リンクとして従来どおり指定画面を開く。
